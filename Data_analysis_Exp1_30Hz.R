...
## 60 Hz data downsampled to 30Hz by taking 
# every 2nd sample
...
P_density_E1=sub %>%
  rename(Corpus = Task_Name)%>%
  mutate(Corpus = recode(Corpus,
                         "Freq_sentences" = "Frequency corpus",
                         "Single_line_sentences" = "Single-line corpus"))%>%
  ggplot(aes(x = error, fill = Dimension)) +
  geom_density(alpha = 0.4) +
  geom_rug(aes(color = Dimension), sides = "b", alpha = 0.6) +
  #facet_wrap(~Corpus+Dimension)+
...
# global_max_abs <- range(
#   c(plot_x$mean_x_deg, plot_y$mean_x_deg, plot_x$mean_y_deg, plot_x$mean_y_deg),
#   na.rm = TRUE
# )
# 
...
write.csv(x = parsed_el, file = 'LAB/data/preproc/EK_eyelink_freq_30Hz.csv')
write.csv(x = parsed_web, file = 'LAB/data/preproc/EK_webcam_freq_30Hz.csv')
...
write.csv(x = words_dat, file = 'LAB/data/target_word_fixation_data_30Hz.csv')
...
write.csv(x = parsed_c_el, 
          file = 'LAB/data/preproc/EK_eyelink_corpus_30Hz.csv')

write.csv(x = parsed_c_web, 
          file = 'LAB/data/preproc/EK_webcam_corpus_30Hz.csv')
...
for(i in 1:length(nsubs)){
  ...
  for(j in 1:length(nitems)){
    ...
    if(length(which(is.na(b$wordID)))== nrow(b)){
      next
    }
...
write.csv(x = corpus_final, file = 'LAB/data/corpus_fixation_data_30Hz.csv')
