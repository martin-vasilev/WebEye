...
webcam <- webcam %>%
  group_by(subject, Task_Name, Trial_Id) %>%
  slice(seq(1, n(), by = 2)) %>%
  ungroup()
...
write.csv(x = parsed_el, file = 'LAB-SPAIN/data/preproc/EK_eyelink_freq_30Hz.csv')
write.csv(x = parsed_web, file = 'LAB-SPAIN/data/preproc/EK_webcam_freq_30Hz.csv')
...
for(i in 1:length(nsubs)){
  ...
  for(j in 1:length(nitems)){
    ...
    if(b$list[1]=="A"){
      ...
      which_row<- which(Corpus_A$index== b$trial[1])
      freq<- Corpus_A$frecA[which_row]
      target<- ifelse(freq== 'low', Corpus_A$word_low[which_row], Corpus_A$word_high[which_row])
      ...
      which_row<- which(Corpus_B$index== b$trial[1])
      freq<- Corpus_B$frecB[which_row]
      target<- ifelse(freq== 'low', Corpus_B$word_low[which_row], Corpus_B$word_high[which_row])
...
write.csv(x = words_dat, file = 'LAB-SPAIN/data/target_word_fixation_data_30Hz.csv')
...
write.csv(all_words_dat, 'LAB-SPAIN/data/all_words_fixation_data_30Hz.csv')
