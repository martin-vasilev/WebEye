...
library(readr)
dat <- read_csv("LAB/data/target_word_fixation_data.csv")

dat<- subset(dat, Tracker== "Eyelink")
...
nsub = seq(10, 100, 10)  # number of subjects
nitems = seq(10, 100, 10) 
data_loss<- 0.1 # percentage of data points to randomly remove


if(file.exists("power_Eyelink.Rda")){
  load("power_Eyelink.Rda")
} else {
  power <- NULL
}
...
for(i in 1:length(nsub)){ # for each subject..
  ...
  for(j in 1:length(nitems)){ # for each item..
   ...
   #  sum_skip<- summary(p_skip)
    ...
    # 
    power<- rbind(power, t)
    save(power, file= "power_Eyelink.Rda")
  }
