...
target_words_exp1 <- read_csv("LAB/data/target_word_fixation_data_30Hz.csv") %>%
  mutate(Freq = factor(Freq, levels = c("low", "high")),
         Tracker = factor(Tracker, levels = c("Webcam", "Eyelink")),
         item = factor(item),
         sub = factor(sub))

corpus_words_exp1 <- read_csv("LAB/data/corpus_fixation_data_30Hz.csv") %>%
...
qs_save(blmm_target_FFD_exp1_gaussian, "LAB/models/brms/blmm_target_FFD_exp1_gaussian_30Hz.qs")
...
summary(blmm_target_GD_exp1_gaussian)

qs_save(blmm_target_GD_exp1_gaussian, "LAB/models/brms/blmm_target_GD_exp1_gaussian_30Hz.qs")
...
qs_save(blmm_target_SFD_exp1_gaussian, "LAB/models/brms/blmm_target_SFD_exp1_gaussian_30Hz.qs")
...
summary(blmm_target_TVT_exp1_gaussian)
qs_save(blmm_target_TVT_exp1_gaussian, "LAB/models/brms/blmm_target_TVT_exp1_gaussian_30Hz.qs")


## Skipping:
blmm_target_skip_exp1 <-
  brm(
    data = target_words_exp1,
    formula = bf(
...
summary(blmm_target_skip_exp1)

qs_save(blmm_target_skip_exp1, "LAB/models/brms/blmm_target_skip_exp1_30Hz.qs")


## corpus analysis

# add column indicating which word (from column word_num) is the final word in the sentence (contains a ".")
corpus_words_exp1 <- corpus_words_exp1 %>% group_by(sub, item) %>% mutate(position = case_when(grepl("\\.$", wordID) & word_num == max(word_num) ~ "last",
                                                                       word_num == 1 ~ "first",
                                                                       TRUE ~ "middle"),
...
qs_save(blmm_corpus_FFD_exp1_gaussian, "LAB/models/brms/blmm_corpus_FFD_exp1_gaussian_30Hz.qs") 
...
qs_save(blmm_corpus_GD_exp1_gaussian, "LAB/models/brms/blmm_corpus_GD_exp1_gaussian_30Hz.qs")
...
qs_save(blmm_corpus_SFD_exp1_gaussian, "LAB/models/brms/blmm_corpus_SFD_exp1_gaussian_30Hz.qs")
...
qs_save(blmm_corpus_TVT_exp1_gaussian, "LAB/models/brms/blmm_corpus_TVT_exp1_gaussian_Hz.qs")
...
blmm_corpus_skip_exp1 <- 
  brm(
    data = corpus_words_exp1 %>% filter(position != "first" & position != "last"),
    formula = bf(
      skip_first_pass ~ ctr(zipf) * Tracker + (Tracker+ctr(zipf)  | sub) + (Tracker | word_code)),
    warmup = 1000,
    iter = 5000,
    chains = 4,
    prior = priors_gaussian,
...
qs_save(blmm_corpus_skip_exp1, "LAB/models/brms/blmm_corpus_skip_exp1_30Hz.qs")
...
target_words_exp1_means_table[,11:12]<- round(target_words_exp1_means_table[,11:12],2)
...
summary(blmm_target_FFD_exp1_gaussian)
BF1 = hypothesis(blmm_target_FFD_exp1_gaussian, hypothesis = 'Freq1 = 0', seed= 1234)
1/BF1$hypothesis$Evid.Ratio

greater0<-hypothesis(blmm_target_FFD_exp1_gaussian, hypothesis = 'Freq1 >0', seed= 1234)

sm

b1 = hypothesis(blmm_target_FFD_exp1_gaussian, hypothesis = 'Freq1 > 0', seed= 1234)
...
ffd_corpus <- qs_read(here("LAB", "models", "brms", "blmm_corpus_FFD_exp1_gaussian.qs"))
summary(ffd_corpus)
