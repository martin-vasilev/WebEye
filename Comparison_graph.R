...
means<- dat_long %>%
  #  select(-Freq)%>% 
  #  drop_na() %>% 
  group_by(Experiment, Freq, measure)%>% 
...
P1 <- ggplot(dat_long, aes(x = Freq, y = duration, color = Freq, fill = Freq)) +
  ggdist::stat_halfeye(
    adjust = .5,
    width = .6,
    .width = 0,
  ...
  facet_wrap(~ Experiment + measure, scales = "free") +
  coord_cartesian(xlim = c(1.2, NA), clip = "off") +
  scale_color_manual(values = pallete1[1:2]) +
  scale_fill_manual(values = pallete1[1:2]) +
  theme_classic(20) +
  ylab("Fixation durations- Webcam data (in ms)") +
...
ggsave(plot = P1, filename = 'LAB/Plots/webcam_effect.png', width = 16,
       height = 16, units = 'in')
